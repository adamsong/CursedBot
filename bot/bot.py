...
async def on_attack(message):
    global attack_stage, current_attack, users, effects_to_roll
    ...
    if attack_stage == 1 and current_attack["attacker"] == message.author.id:
        ...
        if message.content in ["yes", "no"]:
            ...
            attack_stage = 6
            dice = 'd20' if current_attack['advantage'] == 'no' \
                else ('2d20' + ('k1' if current_attack['advantage'] == 'advantage' else 'kl1'))
        ...
        if message.content in ["yes", "no"]:
            ...
            if message.content == "yes":
                if "uses" not in users[current_attack["attacker"]]["weapons"][current_attack['weapon']['name']]:
        ...
        if message.content in ["yes", "no"]:
            ...
            if current_attack["button"] == "yes":
                ...
                await message.channel.send(
                    ...
                    f"{users[current_attack['attacker']]['weapons'][current_attack['weapon']['name']]['uses']}")
...
async def on_attack_roll(message):
    ...
    if attack_stage == 6:
        ...
        if roll_result <= users[current_attack["attacker"]]["weapons"][current_attack['weapon']['name']]["uses"]:
            print("Failed")
            effects_to_roll = max((users[current_attack["attacker"]]["weapons"][current_attack['weapon']['name']][
                                       "uses"] // rolls_per_effect) + 1, 1)
            await message.channel.send("!1d20000")
            attack_stage = 202
        else:
            effect_num = users[current_attack["attacker"]]["weapons"][current_attack['weapon']['name']][
                "current-effect"]
...
def make_summary():
    ...
    if current_attack['weapon']['weapon-type'] == "mundane":
        summary += f"Attack {'hit' if current_attack['hit'] == 'yes' else 'missed'} with " \
                   f"{current_attack['attack-roll']}\n"
    ...
    if current_attack['weapon']['weapon-type'] == "magic" or current_attack['hit'] == "yes":
        known = users[current_attack['attacker']]['weapons'][current_attack['weapon']['name']]['player-known']
        effect_id = users[current_attack['attacker']]['weapons'][current_attack['weapon']['name']]['current-effect']
        translation = table.get_table(int(effect_id))
        summary += f"The effect was: {'Unknown' if known == 'false' else translation}\n"
    return summary
...
async def attack(ctx, *args):
    ...
    if ctx.invoked_subcommand is None:
        if ctx.author.id not in users or "weapons" not in users[ctx.author.id] or len(
                users[ctx.author.id]["weapons"]) == 0:
...
async def check_cascade(ctx):
    global attack_stage
    attack_stage = 201
    users[current_attack['attacker']]['weapons'][current_attack['weapon']['name']][
        'uses'] += 1
    await ctx.send(
        f"Checking for cascade, DC is "
        f"{users[current_attack['attacker']]['weapons'][current_attack['weapon']['name']]['uses']}")
...
async def new(ctx, *args):
    if (len(args) == 3) or (len(args) == 2 and args[1] == "magic"):
        if ctx.author.id not in users:
            users[ctx.author.id] = {}
        if "weapons" not in users[ctx.author.id]:
...
@bot.command(pass_context=True)
async def check(ctx):
    check_string = "Effect Data:\n```\n"
    if ctx.author.id not in users:
        users[ctx.author.id] = {}
    if "weapons" not in users[ctx.author.id] or len(users[ctx.author.id]["weapons"]) == 0:
        await ctx.send("You have no weapons")
        return

    check_string += "\n".join(f"{weapon_in:<30}: {users[ctx.author.id]['weapons'][weapon_in]['uses']}" for weapon_in in
                              users[ctx.author.id]["weapons"])
    check_string += "\n```"
    await ctx.send(check_string)
...
async def effect(ctx, weapon_name=""):
    ...
    if not ctx.invoked_subcommand:
        ...
        elif ctx.author.id not in users or "weapons" not in users[ctx.author.id] or len(
                users[ctx.author.id]["weapons"]) == 0:
            await ctx.send("Please add at least one weapon to your user")
        elif weapon_name not in users[ctx.author.id]['weapons']:
...
@effect.command(pass_context=True)
async def translate(ctx, effect_id: int):
...
async def learn(ctx, member: discord.Member, weapon_name):
    if member.id not in users or "weapons" not in users[member.id] or len(users[member.id]["weapons"]) == 0:
        await ctx.send("Please add at least one weapon to your user")
    elif weapon_name not in users[member.id]['weapons']:
        await ctx.send("Invalid weapon name")
    else:
